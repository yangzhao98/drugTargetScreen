...
datUKBSNP <- data.table::fread(
    paste(path.ukb,"/",
          grep("variant",list.files(path.ukb),value=TRUE),
          sep=""),
    select=selCols
)
head(datUKBSNP[1:5,])

## exposure-GWAS of LDL-C from UKB
UKBCVD.file <- paste(path.ukb,"/",grep("v3.both",list.files(path.ukb),value=TRUE),sep="")
system.time(
  datExpUKB <- dat4UKBNealeLab(
    UKBCVD.file,datUKBSNP,type="exposure",binaryTrait=FALSE
  )
)
datExpUKB <- datExpUKB[datExpUKB$pval.exposure<1e-04,]
datExpUKB$exposure <- "UKB LDL-C"
datExpUKB <- datExpUKB[!is.na(datExpUKB$SNP),]
# head(datExpUKB)
data.table::fwrite(
    drugTargetScreen::dat4GRAPPLE(datExpUKB),
    file=paste(path.res,"/datExpUKB.csv",sep="")
)
head(datExpUKB)

## slection-GWAS of LDL-C from GCLC
colSelected <- c("rsID","METAL_Effect","METAL_StdErr","METAL_Pvalue",
                 "POOLED_ALT_AF","ALT","REF","CHROM","POS_b37","N"
)
datLDLC <- data.table::fread(
    paste(path.gclc,"/",
        grep("LDL_",list.files(path.gclc),value=TRUE)[1],
        sep=""),
    select=colSelected
)
# Select SNPs with p-value<1e-04
datLDLC$METAL_Pvalue <- as.numeric(datLDLC$METAL_Pvalue)
datLDLC <- datLDLC[METAL_Pvalue<=1e-04]
...
## (2) leadning SNPs within each protein-coding gene locus
...
datExpLDLCGene[
  ,`:=`(type=(ifelse(!is.na(external_gene_name) & pos>(start_position-5e5) & pos<(end_position+5e5),
                     "cis-pQTL",
                     "trans-pQTL")))
  ,by=.(SNP)]
...
data.table::fwrite(
    drugTargetScreen::dat4GRAPPLE(datExpLDLCGeneSuSiE),
...
selSNPs <- datExpLDLCGene$SNP[datExpLDLCGene$external_gene_name %in% posCtrl]
...
plt.woCor <- plt.Scatter(datMR,title="All IVs",legend.idx=FALSE)
plt.woCor
...
MRResult4Gene$exp <- "CETP/HMGCR/NPC1L1/PCSK9"
MRResult4Gene[MRResult4Gene$method %in% mainMethod,]
plt.4Gene <- plt.Scatter(
  datMR=datMR[datMR$SNP %in% selSNPs,],
  title="IVs within or near either CETP, HMGCR, NPC1L1, or PCSK9 locus")
plt.4Gene
