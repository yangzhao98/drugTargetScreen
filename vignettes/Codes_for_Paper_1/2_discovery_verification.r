...
dat4Forest[,`:=`(Gene=ifelse(label!="All IVs" & grepl("CETP",exp),"CETP",
                      ifelse(label!="All IVs" & grepl("HMGCR",exp),"HMGCR",
                      ifelse(label!="All IVs" & grepl("NPC1L1",exp),"NPC1L1",
                      ifelse(label!="All IVs" & grepl("PCSK9",exp),"PCSK9","All")))))
           ,by=.(exp)]
dat4Forest[,`:=`(lci=b-1.96*se,uci=b+1.96*se)]
# dat4ForestIVW <- dat4Forest[method %in% mainMethod,]
library(ggplot2)
plt.Forest <- ggplot2::ggplot(dat4Forest[method==mainMethod & Gene !="All"],
                              ggplot2::aes(y=exp(b),ymin=exp(lci),ymax=exp(uci),
                                           x=Gene,col=label)) +
              ...
              ggtitle("Genetically predicted effects of LDL-C on CAD")
plt.Forest
...
plt.4Gene.CAD.coloc <- lapply(
  posCtrl,FUN=function(i) {
    drugTargetScreen::plt.stack.region(
      geneInfo=datPosCtrlNew[external_gene_name==i],
      datOut=datCVDOut,outName="CAD",
      datQTL=dateQTL,qtlName="eQTLGene phase 1",
      datExp=datExpLDLC,expName=paste("LDL-C (",i,")",sep=""),
      ldRef=ldRef)
  }
)
plt.4Gene.CAD.coloc[[1]]
plt.4Gene.CAD.coloc[[2]]
plt.4Gene.CAD.coloc[[3]]
plt.4Gene.CAD.coloc[[4]]